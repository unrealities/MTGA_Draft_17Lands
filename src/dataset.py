...
class Dataset:
    ...
    def get_data_by_name(self, name_list: List[str]) -> List[Dict]:
        ...
        transformed_dataset = {
            v[DATA_FIELD_NAME]: v for v in self._dataset["card_ratings"].values()
        }
    ...
    def get_names_by_id(self, id_list: List[str]) -> List[str]:
        ...
        for arena_id in id_list:
            ...
            if string_id in self._dataset["card_ratings"]:
                name_list.append(
                    self._dataset["card_ratings"][string_id][DATA_FIELD_NAME]
                )

        return name_list

    def get_ids_by_name(
        self, name_list: List[str], return_int: bool = False
    ) -> List[str]:
        """
        ...
        transformed_dataset = {
            v[DATA_FIELD_NAME]: k for k, v in self._dataset["card_ratings"].items()
        }

        for name in name_list:
            if name in transformed_dataset:
                id_list.append(
                    int(transformed_dataset[name])
                    if return_int
                    else transformed_dataset[name]
                )
    ...
    def get_all_names(self) -> List[str]:
        ...
        if self._dataset is not None:
            name_list = list(
                set(
                    [v[DATA_FIELD_NAME] for v in self._dataset["card_ratings"].values()]
                )
            )

        return name_list

    def get_card_archetypes_by_field(
        self, card_name: str, field: str
    ) -> List[Tuple[str, str]]:
        """
        ...
        archetype_list.append(
            ["", "All Decks", win_rate, all_decks.get(WIN_RATE_FIELDS_DICT[field], 0)]
        )
        ...
        for color, name in COLOR_NAMES_DICT.items():
            if color in deck_stats:
                ...
                wr, gc = spec_stats.get(field, 0), spec_stats.get(
                    WIN_RATE_FIELDS_DICT[field], 0
                )
                if wr != 0:
