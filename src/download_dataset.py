...
class DownloadDatasetWindow(ScaledWindow):
    ...
    def enter(self, dataset_args: DatasetArgs = None):
        ...
        self.window.protocol(
            "WM_DELETE_WINDOW", lambda window=self.window: self.__exit(window)
        )
        ...
        try:
            ...
            headers = {
                ...
                "EVENT": {"width": 0.12, "anchor": tkinter.CENTER},
                ...
                "# GAMES": {"width": 0.10, "anchor": tkinter.E},
            }
            ...
            set_label = Label(
                self.window, text="Set:", style="SetOptions.TLabel", anchor="e"
            )
            event_label = Label(
                self.window, text="Event:", style="SetOptions.TLabel", anchor="e"
            )
            start_label = Label(
                self.window, text="Start Date:", style="SetOptions.TLabel", anchor="e"
            )
            end_label = Label(
                self.window, text="End Date:", style="SetOptions.TLabel", anchor="e"
            )
            group_label = Label(
                self.window, text="User Group:", style="SetOptions.TLabel", anchor="e"
            )
            threshold_label = Label(
                self.window, text="Min Games:", style="SetOptions.TLabel", anchor="e"
            )

            draft_choices = LIMITED_TYPE_LIST
            status_text = tkinter.StringVar()
            status_label = Label(
                self.window, textvariable=status_text, style="Status.TLabel", anchor="c"
            )
            ...
            set_entry = OptionMenu(self.window, set_value, set_choices[0], *set_choices)
            menu = self.root.nametowidget(set_entry["menu"])
            menu.config(font=self.fonts_dict["All.TMenubutton"])

            set_value.trace_add(
                "write",
                lambda *args, start=start_entry, selection=set_value, set_list=sets: self.__update_set_start_date(
                    start, selection, set_list, *args
                ),
            )
            set_value.trace_add(
                "write",
                lambda *args, event_widget=event_entry, event_selection=event_value, set_selection=set_value, set_list=sets: self.__update_event_format(
                    event_widget, event_selection, set_selection, set_list, *args
                ),
            )
            draft_groups = LIMITED_GROUPS_LIST
            group_value = tkinter.StringVar(self.root)
            group_entry = OptionMenu(
                self.window, group_value, draft_groups[0], *draft_groups
            )
            menu = self.root.nametowidget(group_entry["menu"])
            menu.config(font=self.fonts_dict["All.TMenubutton"])

            progress = Progressbar(
                self.window, orient=tkinter.HORIZONTAL, length=100, mode="determinate"
            )
            ...
            if dataset_args and not self._auto_add_set_ran:
                set_string = [
                    ...
                    if v.seventeenlands[0] == dataset_args.draft_set
                ]
                if set_string:
    ...
    def __update_event_format(
        self, event_widget, event_selection, set_selection, format_list, *_
    ):
        ...
        try:
            ...
            if set_data.formats:
                ...
                for event_format in set_data.formats:
                    menu.add_command(
                        label=event_format,
                        command=lambda value=event_format: event_selection.set(value),
                    )  # add new ones
    ...
    def _handle_game_count_and_notify(
        self, download_success, game_count, file_list, download_args
    ):
        if download_success and file_list:
            if game_count == 0:
                ...
                for file in file_list:
                    if (
                        set_code == file[0]
                        and download_args.draft.get() == file[1]
                        and download_args.user_group.get() == file[2]
                        and download_args.start.get() == file[3]
                        and (
                            download_args.end.get() == file[4]
                            or download_args.end.get() > file[4]
                        )
                        and game_count == file[5]
                    ):
                        notify = True
                        break
                if notify:
