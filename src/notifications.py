...
NOTIFICATION_DATASET_MISSING_CHECK_LIST = [
    LIMITED_TYPE_STRING_DRAFT_QUICK,
    LIMITED_TYPE_STRING_DRAFT_BOT,
]
...
class Notifications:
    ...
    def __init__(
        ...
        self.file_location = ""
        self.expansions = (
            {v.set_code: k for k, v in expansions.data.items()} if expansions else None
        )
    ...
    def check_and_pull_recent_sets(self):
        ...
        """
        if (
            self.configuration.card_data.last_auto_check != 0
            or self.configuration.card_data.latest_dataset != ""
        ):
            return
    ...
    def check_for_missing_dataset(self, expansion_code: str, event_type: str):
        ...
        try:
            if (
                not expansion_code
                or not self.expansions
                or expansion_code not in self.expansions
                or event_type not in NOTIFICATION_DATASET_MISSING_CHECK_LIST
                or self.dataset_window.check_instance_open()
            ):
                return
            ...
            if tkinter.messagebox.askyesno(
                title="Missing Dataset", message=message_string
            ):
    ...
    def update_dataset(self):
        try:
            ...
            time_difference = (
                current_time - self.configuration.card_data.last_auto_check
            )
            ...
            if not dataset_info:
                return
            logger.info(
                f"Checking dataset {self.configuration.card_data.latest_dataset} for updates"
            )
            ...
            if not self.expansions or dataset_info[0] not in self.expansions:
                return
            if game_count > dataset_info[5]:
                ...
                if tkinter.messagebox.askyesno(
                    title="Dataset Update Available", message=message_string
                ):
                    dataset_args = DatasetArgs(
    ...
    def _update_executable(self) -> bool:
        ...
        message_string = (
            f"Version {self.new_version} is now available. Would you like to upgrade?"
        )
