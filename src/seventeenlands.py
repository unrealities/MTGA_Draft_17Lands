...
class Seventeenlands():
    ...
    def download_card_ratings(self, set_code, colors, draft, start_date, end_date, user_group, card_data):
        from src.utils import normalize_color_string
        url = self.build_card_ratings_url(set_code, draft, start_date, end_date, user_group, colors)
        response = requests.get(url, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        self.process_card_ratings(normalize_color_string(colors), response.json(), card_data)

    def download_color_ratings(self, set_code, draft, start_date, end_date, user_group, color_filter=None):
        url = self._build_color_ratings_url(set_code, draft, start_date, end_date, user_group)
        response = requests.get(url, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        return self._process_color_ratings(response.json(), color_filter or COLOR_FILTER)
            ...
            try:
                ...
                for data_field in DATA_FIELD_17LANDS_DICT[DATA_SECTION_IMAGES]:
                    if data_field in card and card[data_field]:
                        img = f"{URL_17LANDS}{card[data_field]}" if card[data_field].startswith(IMAGE_17LANDS_SITE_PREFIX) else card[data_field]
                ...
                for key, value in DATA_FIELD_17LANDS_DICT.items():
                    if key == DATA_SECTION_IMAGES: continue
                    if value in card:
        ...
        for entry in colors_json:
            try:
                ...
                if entry.get("games", 0) >= COLOR_WIN_RATE_GAME_COUNT_THRESHOLD:
                    winrate = round((float(entry.get("wins", 0)) / entry.get("games", 1)) * 100, 1)
