...
def test_dropdown_percentage_mapping_order_invariance(tmp_path):
    # 1. Setup mock data
    dummy_cards = {str(i): {"name": f"c{i}", "deck_colors": {"All Decks": {"gihwr": 0.0}}} for i in range(101)}
    mock_data = {
    ...
    try:
        ...
        with patch('src.overlay.read_configuration', return_value=(conf, True)), \
             ...
             patch('src.overlay.ArenaScanner.retrieve_data_sources', return_value={"Source": str(data_file)}), \
