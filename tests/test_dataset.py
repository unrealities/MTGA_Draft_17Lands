...
OTJ_GET_IDS_BY_NAME_TESTS_PASS = [
    (
        ["Rest in Peace", "Thoughtseize", "Djinn of Fool's Fall", "Slick Sequence"],
        False,
        ["87050", "90718", "90389", "90579"],
    ),
    (
        ["Rest in Peace", "Thoughtseize", "Djinn of Fool's Fall", "Slick Sequence"],
        True,
    ...
    (
        ["Brazen Borrower", "Fake Card", "Mentor of the Meek", "Crime /// Punishment"],
        False,
        ["90652", "90737"],
    ),
    (
...
OTJ_GET_NAMES_BY_ID_TESTS_PASS = [
    ...
    ([70186, 90737], ["Brazen Borrower", "Crime /// Punishment"]),
    (
        ["73807", "73905", "ABCD", 90579],
        ["Rest in Peace", "Thoughtseize", "Slick Sequence"],
    ),
    (
        ["73807", "73905", "", 90579],
        ["Rest in Peace", "Thoughtseize", "Slick Sequence"],
...
OTJ_GET_DATA_BY_ID_TEST_PASS = [
    ...
    (
        ["73905", 90389],
        [
            {"name": "Thoughtseize", "cmc": 1, "mana_cost": "{B}"},
            {"name": "Djinn of Fool's Fall", "cmc": 5, "mana_cost": "{4}{U}"},
        ],
    ),
    ([], []),
]
...
OTJ_GET_DATA_BY_NAME_TEST_PASS = [
    ...
    (
        ["Thoughtseize", "Djinn of Fool's Fall"],
        [
            {"name": "Thoughtseize", "cmc": 1, "mana_cost": "{B}"},
            {"name": "Djinn of Fool's Fall", "cmc": 5, "mana_cost": "{4}{U}"},
        ],
...
@pytest.mark.parametrize(
    "name_list, return_int, expected_ids", OTJ_GET_IDS_BY_NAME_TESTS_PASS
)
...
def test_otj_get_data_by_id(otj_dataset, id_list, expected_data):
    ...
    for i in range(len(data_list)):
        ...
        assert all(
            data_list[i].get(key) == expected_data[i].get(key)
            for key in data_list[i].keys() & expected_data[i].keys()
        ), f"Get Data by ID: Collected:{data_list[i]}, Expected:{expected_data[i]}"
...
def test_get_card_archetypes_by_field_fail_invalid_name(otj_dataset):
    ...
    """
    archetype_list = otj_dataset.get_card_archetypes_by_field(
        "FakeCard", DATA_FIELD_GIHWR
    )
...
def test_get_card_archetypes_by_field_fail_empty(otj_dataset):
    ...
    """
    archetype_list = otj_dataset.get_card_archetypes_by_field(
        "Rest in Peace", DATA_FIELD_GIHWR
    )
    assert not archetype_list
...
def test_get_card_archetypes_by_field_pass(otj_dataset):
    ...
    """
    archetype_list = otj_dataset.get_card_archetypes_by_field(
        "Hardbristle Bandit", DATA_FIELD_GIHWR
    )
...
def test_dataset_normalization_on_load(tmp_path):
    ...
    bad_data = {
        "color_ratings": {"GW": 55.0, "RW": 52.0},  # Should be WG  # Should be WR
        "card_ratings": {
            "1": {
                "name": "Test Card",
                "deck_colors": {
    ...
    with patch(
        "src.dataset.check_file_integrity", return_value=(Result.VALID, bad_data)
    ):
        ds = Dataset()
